...
public class Level extends HUD {
	...
	private Enemy enemy=new Enemy(ResourceManager.getBitmap(R.drawable.enemy),100f,200f,0f,direction,2f,100,false);
	public int bites = 0;
	...
	public void collisionCheck(){
		...
		//Added these lines for shooting.
		for (int i = 0; i < player.heldWeapon.bullet.size(); i++) {
				if((player.heldWeapon.bullet.get(i).position.X-enemy.rect.X)<50){
					Log.d(TAG, "Bullet position X " + player.heldWeapon.bullet.get(i).position.X);
					Log.d(TAG, "Bullet position Y " + player.heldWeapon.bullet.get(i).position.Y);
					if(player.heldWeapon.bullet.get(i).position.Y-enemy.rect.Y<50){
						enemy.speed = 0f;
						...
						enemy.rect.Y =0;
						enemy.rect.width =0f;
						enemy.rect.height =0f;
						player.heldWeapon.bullet.get(i).clear();
					}
				}
		}
	...
	public void restart(){
		Log.d(TAG,"restart" + bites);
		 enemy=new Enemy(ResourceManager.getBitmap(R.drawable.enemy),100f,200f,0f,direction,2f,100,false);
